...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] Vector2 footStepPitchRange = new Vector2(1.2f, 1.4f);
    Vector3 localScale;
    ...
    void MovePlayer()
    {
        ...
        if (Mathf.Abs(rb.velocity.x) > stoppedThreshold || Mathf.Abs(rb.velocity.y) > stoppedThreshold) {
            var source = GetComponent<AudioSource>();
            if (!source.isPlaying) {
                source.pitch = Random.Range(footStepPitchRange.x, footStepPitchRange.y);
                source.Play();
            }
        }
